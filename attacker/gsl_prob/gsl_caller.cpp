...
enum Dist_Type { Ber = 0, Bin, Geo, Nbin, Pois, Hyper, Unif, Norm };
...
/* Computes the probability p(x) of obtaining k from a normal distribution */
void norm_get_prob (double x, double sigma) {

	cout.precision(dbl::max_digits10);
	cout << fixed << gsl_ran_gaussian_pdf(x, sigma) << "\n";	
}

void norm_get_prob_P (double x, double sigma) {

	cout.precision(dbl::max_digits10);
	cout << fixed << gsl_cdf_gaussian_P(x, sigma) << "\n";
}
...
int main(int argc, char *argv[]) {
	...
	if (argc < 2) {
		...
		switch (dt)
		{
			...
			case Norm:
			{
				if (check_papameters(Norm, argc, argv)) {
					double x = stod(argv[2]);
					double mu = stod(argv[3]);
					double sigma = stod(argv[4]);
					unsigned int number = stoi(argv[5]);
					mu = number * mu;
					sigma = sqrt(number * sigma * sigma);
					x = number * x;
					norm_get_prob_P((x-mu),sigma);
				}
				else {
					// release the generator
					gsl_rng_free (r);
